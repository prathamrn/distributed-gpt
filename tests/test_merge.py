"""Protects the arithmetic and the rules that decide what a round produces: how deltas are combined"""
import math
...
def sd(*vals):
    """- Build a tiny two-tensor state dict standing in for a model's parameters."""
...
def test_weighted_average_equal_weights_is_mean():
    """- Equal-step workers must average plainly; a weighting bug here would silently bias every merged model."""
...
def test_weighted_average_by_steps():
    """- Weighting by n_steps makes a small partial delta count less than a full one."""
...
def test_weighted_average_casts_bf16_to_fp32():
    """- Deltas arrive in bf16 when delta_dtype is set, which halves the upload"""
...
def test_median_ignores_single_outlier():
    """- The coordinate-wise median ignores one wild delta that a mean would absorb."""
...
def test_trimmed_mean_drops_extremes():
    """- The trimmed mean discards the extremes on each side before averaging."""
...
def test_aggregate_dispatch():
    """- The aggregation run setting is a string from the command line"""
...
def test_outer_state_roundtrip():
    """- The outer optimizer's momentum buffer survives a state_dict round trip, as it must through ckpt.pt."""
...
def test_delta_sign_convention():
    """- A delta is served weights minus trained weights, so applying it as W - delta reproduces the trained weights."""
...
def view(**kw):
    """- Build a RoundView with three alive workers and nothing reported, overridable per test."""
...
def test_never_closes_with_zero_deltas():
    """- However long a round has been open, it never closes with nothing to merge."""
    assert should_close_round(view(elapsed_s=999))[0] is False


def test_closes_when_all_alive_reported():
    """- The fast path: when everyone alive has reported there is nothing left to wait for"""
    assert should_close_round(view(reported={"w1", "w2", "w3"}))[0] is True


def test_dead_worker_does_not_block_close():
    """- Once a worker has been reaped it is no longer in the alive set, so the round closes on the survivors."""
...
def test_timeout_respects_min_workers():
    """- min_workers is the floor on how many deltas a timeout close may merge."""
...
def test_round_timeout_uses_median_with_floor_and_initial():
    """- No history uses the initial value; otherwise factor x median, never below the floor."""
...
def test_adaptive_k():
    """- K_i scales with speed_i / median_speed; unmeasured workers get the base K."""
...
def test_adaptive_k_subtracts_transfer_overhead():
    """- A worker's own transfer time is subtracted from its budget, with a floor so it never drops to one step (R16)."""
