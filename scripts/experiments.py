...
PORT = 8000
os.chdir(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))   # results/ paths are relative to the repo root
...
def start_workers(n: int, threads: int, device: str, out_dir: str) -> list[subprocess.Popen]:
    ...
    for i in range(1, n + 1):
        ...
        procs.append(subprocess.Popen([sys.executable, "-m", "dgpt.worker", "--name", f"worker-{i}", "--coordinator", f"http://127.0.0.1:{PORT}",
...
def run_one(name: str, e: dict) -> dict | None:
    ...
    args = [sys.executable, "-m", "dgpt.coordinator", "--run-name", name, "--port", str(PORT), "--exit-when-done", "--fresh"]
    for s in e["sets"]:
        args += ["--set", s]
    for t in e["train"]:
        args += ["--train-set", t]
...
def ledger(path: str = "evals.md"):
    ...
    lines = ["# Eval ledger", "", "Every training run, why it was run, and what it showed. Regenerated by",
             "`python3 scripts/experiments.py --ledger`; reasons live in `experiments.py` (REASONS / SINGLE_RUNS).",
